...
public class Player {
    ...
    public void moveLeft(OrthographicCamera camera, SpriteBatch batch) {
        ...
        if(animationPlaying){
            ...
            batch.draw(walkingLeft.getKeyFrame(animationTime  += Gdx.graphics.getDeltaTime()), 400, 400);
            if(walkingLeft.isAnimationFinished(animationTime)){
            	animationPlaying = false;
            	leftMode = true;
            	frontMode = false;
            	rightMode = false;
	...
	public void shoot(OrthographicCamera camera, SpriteBatch batch) {
		StandardFireBall SFB = new StandardFireBall();
		if(frontMode){
    		setGameToNormal(TextureShootDown, batch);
    		//SFB.animationBottom(camera, batch);     
    	}else if(rightMode){
    		setGameToNormal(TextureShootRight, batch);
    		//SFB.animationRight(camera, batch);
    		
    	}else if(leftMode){
    		setGameToNormal(TextureShootLeft, batch);
    		SFB.moveLeft(camera, batch);

            
    	}else if(upMode){
    		setGameToNormal(TextureShootUp, batch);
    		//SFB.animationTop(camera, batch);
    	}
